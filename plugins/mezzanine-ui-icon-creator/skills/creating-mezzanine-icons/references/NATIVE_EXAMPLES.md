...
### Open arc band + radial caps — `SpinnerIcon`

The reference for any partial-ring / arc icon (wifi, gauge, loading). A ~3/4 ring of 1u
thickness whose two open ends are closed with **radial straight caps**: `...8 2V3...` caps
the top end and `...3 8H2...` caps the left end. Those `V`/`H` segments run from the outer
radius to the inner radius along a single ray — the flat, perpendicular terminal that defines
the set's clean sans-serif feel. Copy this closure discipline (see PATH_RECIPES §4b); never
let an arc band taper into a wedge.
